...
public class Practice12PathEffectView extends View {
  ...
  @Override protected void onDraw(Canvas canvas) {
    ...
    // 第三处：DashPathEffect
    paint.setPathEffect(new DashPathEffect(new float[] { 10, 5, 15, 2 }, 0));
    canvas.drawPath(path, paint);
    canvas.restore();
    ...
    dashPath.close();
    paint.setPathEffect(
        new PathDashPathEffect(dashPath, 40, 15, PathDashPathEffect.Style.TRANSLATE));
    canvas.drawPath(path, paint);
    canvas.restore();
    ...
    // 第五处：SumPathEffect
    DashPathEffect dashPathEffect = new DashPathEffect(new float[]{10,5,10,5},7);
